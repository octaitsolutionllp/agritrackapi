...
BEGIN
    ...
    DECLARE @TotalExpense DECIMAL(12,2) =
        ISNULL((
            ...
            WHERE e.UserId = @UserId AND (@FarmId IS NULL OR f.FarmId = @FarmId)
        ), 0) +
        ISNULL((
            SELECT SUM(a.Cost)
            ...
            JOIN agritrack.Fields f ON f.Id = cc.FieldId
            WHERE a.UserId = @UserId AND a.Cost IS NOT NULL AND (@FarmId IS NULL OR f.FarmId = @FarmId)
        ), 0);
    ...
    DECLARE @TotalIncome DECIMAL(12,2) = ISNULL((
        ...
        JOIN agritrack.Fields f ON f.Id = cc.FieldId
        WHERE h.UserId = @UserId AND (@FarmId IS NULL OR f.FarmId = @FarmId)
    ), 0);
